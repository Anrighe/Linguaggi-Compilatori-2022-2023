...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // external
      // proto
      char dummy5[sizeof (PrototypeAST*)];

      // program
      // top
      char dummy6[sizeof (RootAST*)];
...
switch (yykind)
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
...
#endif
      {
        YY_ASSERT (tok == token::TOK_END
                   || (token::TOK_YYerror <= tok && tok <= token::TOK_COLON));
      }
#if 201103L <= YY_CPLUSPLUS
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      static
      symbol_type
      make_ENDEXPR (location_type l)
      {
        return symbol_type (token::TOK_ENDEXPR, std::move (l));
      }
#else
      static
      symbol_type
      make_ENDEXPR (const location_type& l)
      {
        return symbol_type (token::TOK_ENDEXPR, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_COMPARE (location_type l)
      {
        return symbol_type (token::TOK_COMPARE, std::move (l));
      }
#else
      static
      symbol_type
      make_COMPARE (const location_type& l)
      {
        return symbol_type (token::TOK_COMPARE, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
